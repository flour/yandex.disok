...
namespace Ya.D.ViewModels
{
    public class DetailPageViewModel : BaseItemsViewModel
    {
        ...
        public bool Shared { get => _shared; set { if (Set(ref _shared, value)) { ChangeSharedState(); } } }
        public string Location { get => _location; set => Set(ref _location, value); }
        public List<DiskItem> DiskItems { get => _items; set => Set(ref _items, value); }
        ...
        {
            Task.Factory.StartNew(async () => await ChangeSharedStateAsync(), CancellationToken.None, TaskCreationOptions.None, DiskService.Client.Sync);
        }

        private async Task ChangeSharedStateAsync()
        {
            DiskURL diskURL;
            if (string.IsNullOrEmpty(SelectedItem.PublicURL) && Shared ||
